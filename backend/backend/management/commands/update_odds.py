...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        try:
            ...
            self.stderr.write(self.style.ERROR(f"Failed to fetch odds: {exc}"))
            return
        ...
        with transaction.atomic():
            for entry in odds_data:
                ...
                try:
                    ...
                    self.stdout.write(
                        f"No unlocked match found for {home_name} vs {away_name} — skipping."
                    )
                    skipped += 1
                    continue
                    ...
                    self.stderr.write(
                        self.style.WARNING(
                            f"Multiple unlocked matches for {home_name} vs {away_name} — skipping."
                        )
                    )
                    skipped += 1
                    continue
                ...
                updated += 1
                self.stdout.write(
                    f"Updated {home_name} vs {away_name}: "
                    f"H={match.odds_home} D={match.odds_draw} A={match.odds_away}"
                )

        self.stdout.write(
            self.style.SUCCESS(f"Done. {updated} match(es) updated, {skipped} skipped.")
        )
